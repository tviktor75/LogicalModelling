...
JSONSerializer.prototype.fromXML=function(/*:Document*/ xmltext)
{
  ...
  for (var i=0; i<figures.length; i++) {
	...
	var lb = allPorts[conns[k].getAttribute('linebegin')];
	var le = allPorts[conns[k].getAttribute('lineend')];
...
JSONSerializer.prototype.fromJSON=function(js)
{
  ...
  for (var i=0; i<figures.length; i++) {
	...
	var lb = allPorts[conns[k]['linebegin']];
	var le = allPorts[conns[k]['lineend']];
        var xxx = new CommandConnect(src.parentNode.workflow, src, dst);
	var conn = new ContextmenuConnection();
	if (conns[k]['color']) {
	  conn.setColor(eval("new Color(" + conns[k]['color'] + ")"));
	}
	if(conns[k]['linebegin']){
		conn.setSourceDecorator(new ArrowConnectionDecorator() );
	}
	if(conns[k]['lineend']){
		conn.setTargetDecorator(new ArrowConnectionDecorator() );
	}
	if(conns[k]['router']) {
		//window.alert(conns[k]['router']);
		conn.setRouter(eval("new " + conns[k]['router'] + "()") );
	}
	xxx.setConnection(conn);
	src.parentNode.workflow.getCommandStack().execute(xxx);
      }
    ...
    for(j=0; j<annotations.length; j++) {
      ...
      var dx = obj.getX();
      var dy = obj.getY();
      obj.addAnnotation(idx[ann['for']], ann['text'], ann['x']-dx-1, ann['y']-dy-1,ann['w'],ann['h']);
    }
  }
